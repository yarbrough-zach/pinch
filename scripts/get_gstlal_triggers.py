...
class GstlalTriggers:
    ...
    def find_gstlal_edward_trigger_files_per_chunk(self, start, end, input_file_path):
        start = int(str(start)[:5])
        
        file_path = f"{input_file_path}/{start}"
        files = glob.glob(f"{file_path}/*LLOID-*.xml.gz")

        files = [file for file in files if int(file.split('-')[-2]) < end]
        print(files)
        return list(files)
...
#    print("Trigger df:", trigger_df)
    ...
    files = gstlalTriggers.find_gstlal_edward_trigger_files_per_chunk(args.start, args.end, args.input_path)
    
    for file in files:
#        print(file)
        gstlalTriggers.read_snglrow(trigger_df, file)

    trigger_df.to_csv(f"{args.output_path}/gstlal_chunk{args.chunk}_triggers.csv")
